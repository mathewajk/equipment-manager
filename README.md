...
Equipment management system for GLTC.
...
```bash
conda create python=3.8 -n equipmentmanager
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate equipmentmanager
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///equipmentmanager` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE equipmentmanager WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE equipmentmanager TO ;
```
...
ERROR:  database "equipmentmanager" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists equipmentmanager
...
```bash
dropdb --if-exists equipmentmanager
